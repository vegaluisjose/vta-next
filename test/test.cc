...
int main() {
  ...
  instr_mem[0].dram_base =
      VTAMemGetPhyAddr(data_buf) / VTA_ACC_ELEM_BYTES + inp_offset;
  ...
  instr_mem[2].dram_base =
      VTAMemGetPhyAddr(data_buf) / VTA_OUT_ELEM_BYTES + out_offset;
  ...
  for (int i = 0; i < (VTA_ACC_ELEM_BYTES / 4); i++) {
    printf("i[%02d]:%08x\n", i,
           mem32[i + (VTA_ACC_ELEM_BYTES / 4) * inp_offset]);
  }
